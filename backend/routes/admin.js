...
router.get('/dashboard', authRequired, requirePermission('can_view_revenue'), (req, res) => {
  const totalUsers = db.prepare('SELECT COUNT(*) c FROM users').get().c;
  const totalBooks = db.prepare("SELECT COUNT(*) c FROM books WHERE status='published'").get().c;
  const sales = db.prepare("SELECT COUNT(*) c, COALESCE(SUM(amount),0) s FROM purchases WHERE status='success'").get();
  ...
  const failed = db.prepare("SELECT COUNT(*) c FROM purchases WHERE status='failed'").get().c;
  const refunded = db.prepare("SELECT COUNT(*) c FROM purchases WHERE status='refunded'").get().c;
...
// ---------- TRANSACTIONS ----------
router.get('/transactions', authRequired, requirePermission('can_view_revenue'), (req, res) => {
...
router.post('/transactions/:id/refund', authRequired, requirePermission('can_manage_payments'), (req, res) => {
  ...
  db.prepare("UPDATE purchases SET status = 'refunded' WHERE id = ?").run(purchase.id);
  logActivity(req.user.id, `A remboursé la transaction ${purchase.id}${revokeAccess ? ' (accès retiré)' : ''}`, purchase.id);
  res.json({ ok: true });
});

// ---------- GESTION DES UTILISATEURS ----------
router.get('/users', authRequired, requireRole('admin_users'), (req, res) => {
  const { q } = req.query;
...
router.get('/users/:id', authRequired, requireRole('admin_users'), (req, res) => {
  const user = db.prepare('SELECT id,name,email,phone,role,status,created_at FROM users WHERE id = ?').get(req.params.id);
...
router.post('/users/:id/suspend', authRequired, requireRole('admin_users'), (req, res) => {
  ...
  logActivity(req.user.id, `A suspendu le compte de ${target.name}`, target.id);
  res.json({ ok: true });
});

router.post('/users/:id/reactivate', authRequired, requireRole('admin_users'), (req, res) => {
  db.prepare("UPDATE users SET status = 'active' WHERE id = ?").run(req.params.id);
  logActivity(req.user.id, 'A réactivé un compte', req.params.id);
  res.json({ ok: true });
});

// ---------- GESTION DES ADMINISTRATEURS (proprietaire uniquement, sauf permission) ----------
router.get('/admins', authRequired, requireRole(), (req, res) => {
  // requireRole() sans argument => seul 'owner' passe, ou permission can_manage_admins
...
router.post('/admins', authRequired, requirePermission('can_manage_admins'), (req, res) => {
  ...
  if (permissions) {
    db.prepare(`INSERT INTO admin_permissions (user_id, can_view_revenue, can_manage_payments, can_manage_admins, can_manage_prices)
      VALUES (?,?,?,?,?)`).run(
      id,
  ...
  logActivity(req.user.id, `A créé un administrateur (${role}) : ${name}`, id);
  res.status(201).json({ id, name, role });
});

router.delete('/admins/:id', authRequired, requirePermission('can_manage_admins'), (req, res) => {
  const target = db.prepare('SELECT * FROM users WHERE id = ?').get(req.params.id);
  ...
  db.prepare('DELETE FROM admin_permissions WHERE user_id = ?').run(req.params.id);
  logActivity(req.user.id, `A retiré les droits admin de ${target.name}`, target.id);
  res.json({ ok: true });
});

// ---------- JOURNAL D'ACTIVITE ----------
router.get('/activity-log', authRequired, requireRole(), (req, res) => {
  const rows = db.prepare(`
...
// ---------- SIGNALEMENTS ----------
router.get('/reports', authRequired, requireRole('admin_content', 'admin_validation'), (req, res) => {
  const rows = db.prepare(`
